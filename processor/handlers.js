...
const createAsset = (asset, owner, vehicle_info, state) => {
  ...
  return state.get([address])
    .then(entries => {
      ...
      return state.set({
        ...
        */
        [address]: encode({ name: asset, owner, vehicle_info })
...
// Add a new owner to state
const addVehicleOwner = (asset,owner, owner_info, state) => {
  const address = getAssetAddress(asset)
  return state.get([address])
    .then(entries => {
      const entry = entries[address]
      ...
      return state.set({
        ...
        This place handles the logic for submission
        */
        [address]: encode({ name: asset,owner, owner_info })
        //[address]: encode({name: asset, owner})
      })
    })
}
...
class JSONHandler extends TransactionHandler {
  ...
  apply(txn, state) {
    ...
    const { action, asset, owner, vehicle_info,owner_info } = JSON.parse(txn.payload)
    ...
    if (action === 'create') return createAsset(asset, signer, vehicle_info, state)
    if (action === 'add_vehicle_owner') return addVehicleOwner(asset, signer, owner_info, state)  //look here
